...
const FormComponent: React.FC = () => {
  ...
  const [ctaBtn, setCtaBtn] = useState<CtaBtn | null>(null)

  useEffect(()=>{
    (async() => {
      const ctaBtnRes = await getWebContent()
      setCtaBtn(ctaBtnRes.data.ctaBtn)
    })()
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let key in ctaBtn) {
      console.log(ctaBtn[key as keyof typeof ctaBtn]);
      formData.append(key, ctaBtn[key as keyof typeof ctaBtn]);
    }
  
    formData.append("tag", "ctaBtn");
