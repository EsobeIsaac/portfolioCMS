...
const AchievementIntroComponent: React.FC = () => {

  const context = useContext(AlertContext)

  interface AchievementsIntro {
    [index: string] : string
  }

  const [achievementIntro, setAchievementIntro] = useState<AchievementsIntro | null>(null)

  useEffect(()=>{
    (async() => {
      const achievementIntroRes = await getWebContent()
      setAchievementIntro(achievementIntroRes.data.achievement)
    })()
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let key in achievementIntro) {
      console.log(achievementIntro[key as keyof typeof achievementIntro]);
      formData.append(key, achievementIntro[key as keyof typeof achievementIntro]);
    }
  
    formData.append("tag", "achievement");
    ...
    try {
      ...
      setAchievementIntro(res.data.data)
      setRequesting(false)
    } catch (err: any) {
      if (err.response) {
        if(context?.alert) {
  ...
  return (
    achievementIntro ? (
      <form onSubmit={handleSubmit}>
        <h2 className='text-md md:text-xl font-semibold text-slate-700 mb-5'>ACHIEVEMENTS INTRO SECTION:</h2>

      <TextInput
        label="Title"
        name="title"
        value={achievementIntro.title}
        onChange={handleInputChange}
      />
      <TextArea
        label="Description"
        name="description"
        value={achievementIntro.description}
        onChange={handleInputChange}
      />
...
export default AchievementIntroComponent;
