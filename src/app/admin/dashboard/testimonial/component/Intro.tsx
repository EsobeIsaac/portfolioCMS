...
const TestimonialIntroComponent: React.FC = () => {

  const context = useContext(AlertContext)

  interface TestimonialIntro {
    title : string
  }

  const [testimonialIntro, setTestimonialIntro] = useState<TestimonialIntro>({title: ' '})

  useEffect(()=>{
    (async() => {
      const TestimonialIntroRes = await getWebContent()
      setTestimonialIntro(TestimonialIntroRes.data.testimonial)
    })()
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("title", testimonialIntro.title);
    formData.append("tag", "testimonial");
    ...
    try {
      ...
      setTestimonialIntro(res.data.data)
      setRequesting(false)
    } catch (err: any) {
      if (err.response) {
        if(context?.alert) {
...
export default TestimonialIntroComponent;
