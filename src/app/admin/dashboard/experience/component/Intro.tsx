...
const ExperienceIntroComponent: React.FC = () => {
  ...
  interface experienceIntro {
    title : string
  }

  const [experienceIntro, setExperienceIntro] = useState<experienceIntro>({title: ' '})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("title", experienceIntro.title);
    formData.append("tag", "experience");
    ...
    try {
      ...
      setExperienceIntro(res.data.data)
      setRequesting(false)
    } catch (err: any) {
      if (err.response) {
        if(context?.alert) {
  ...
  return (
    experienceIntro ? (
      <form onSubmit={handleSubmit}>
        <h2 className='text-md md:text-xl font-semibold text-slate-700 mb-5'>EXPERIENCE INTRO SECTION:</h2>

      <TextInput
        label="Title"
        name="title"
        value={experienceIntro.title}
